...
public class LowRankParam {
	...
	public void putEntry(int h, int m, int hc, int mc, int dd, int label, int t, int hl, int ml, double v)
	{
		list.add(new TMultiwayEntry(h, m, hc, mc, dd, label, t, hl, ml, v));
	}
...
//			int split = 2;
	...
	public void decomposeTMultiway()
	{
		int maxRank = pn.rank;
		...
		for (int i = 0; i < maxRank; ++i) {
			...
			double[] l = options.learnLabel ? new double[lpn.featureSize] : null;
			double[] t = new double[tpn.featureSize];
			double[] hl = options.lexical ? new double[hlpn.featureSize] : null;
			double[] ml = options.lexical ? new double[mlpn.featureSize] : null;
...
//			double normh = Math.cbrt(norml - normt);
			...
			if (options.lexical) {
				for (int j = 0; j < hlpn.featureSize; ++j)
					if (hlpn.isActive[j]) 
						hl[j] = rnd.nextDouble() - 0.5;
				for (int j = 0; j < mlpn.featureSize; ++j)
					if (mlpn.isActive[j]) 
						ml[j] = rnd.nextDouble() - 0.5;
				Utils.normalize(hl, normLex);
				Utils.normalize(ml, normLex);
			}

			int iter = 0;
			double typoNorm = 0.0, multiNorm = 0.0; 
			double lastTypoNorm = Double.POSITIVE_INFINITY, lastMultiNorm = Double.POSITIVE_INFINITY;
			for (iter = 0; iter < MAXITER; ++iter) {
				...
				if (options.lexical) {
					for (MatrixEntry ee : list) {
						TMultiwayEntry e = (TMultiwayEntry)ee;
						e.delexScore = e.value * m[e.m] * h[e.h] * hc[e.hc] * mc[e.mc] * dd[e.dd] * (e.label >= 0 ? l[e.label] : 1.0) * t[e.t];
					}
					...
					for (MatrixEntry ee : list) {
						TMultiwayEntry e = (TMultiwayEntry)ee;
						hl[e.hl] += ml[e.ml] * e.delexScore;
					}
					for (int j = 0; j < i; ++j) {
						double dot = delexSum[j] * Utils.dotsum(ml, mlpn.param[j]);
						for (int k = 0; k < hlpn.featureSize; ++k)
							hl[k] -= dot * hlpn.param[j][k];
					}
					Utils.normalize(hl, normLex);
					...
					for (MatrixEntry ee : list) {
						TMultiwayEntry e = (TMultiwayEntry)ee;
						ml[e.ml] += hl[e.hl] * e.delexScore;
					}
					for (int j = 0; j < i; ++j) {
						double dot = delexSum[j] * Utils.dotsum(hl, hlpn.param[j]);
						for (int k = 0; k < mlpn.featureSize; ++k)
							ml[k] -= dot * mlpn.param[j][k];
					}
					Utils.normalize(ml, normLex);
					...
					for (MatrixEntry ee : list) {
						TMultiwayEntry e = (TMultiwayEntry)ee;
						double sum = hl[e.hl] * ml[e.ml];
						e.lexScore = sum;
					}
					
					for (int j = 0; j < i; ++j) {
						double sum = 0.0;
						sum += Utils.dotsum(hl, hlpn.param[j])
								* Utils.dotsum(ml, mlpn.param[j]);
						lexSum[j] = sum;
					}
				}
				...
				for (MatrixEntry ee : list) {
					TMultiwayEntry e = (TMultiwayEntry)ee;
					h[e.h] += e.value * e.lexScore * m[e.m] * hc[e.hc] * mc[e.mc] * dd[e.dd] * (e.label >= 0 ? l[e.label] : 1.0) * t[e.t];
				}
				for (int j = 0; j < i; ++j) {
					double dot = lexSum[j]
							   * Utils.dotsum(m, mpn.param[j]) 
				...
				for (MatrixEntry ee : list) {
					TMultiwayEntry e = (TMultiwayEntry)ee;
					m[e.m] += e.value * e.lexScore * h[e.h] * hc[e.hc] * mc[e.mc] * dd[e.dd] * (e.label >= 0 ? l[e.label] : 1.0) * t[e.t];
				}
				for (int j = 0; j < i; ++j) {
					double dot = lexSum[j]
							   * Utils.dotsum(h, hpn.param[j]) 
				...
				for (MatrixEntry ee : list) {
					TMultiwayEntry e = (TMultiwayEntry)ee;
					dd[e.dd] += e.value * e.lexScore * h[e.h] * m[e.m] * hc[e.hc] * mc[e.mc] * (e.label >= 0 ? l[e.label] : 1.0) * t[e.t];
				}
				for (int j = 0; j < i; ++j) {
					double dot = lexSum[j]
							   * Utils.dotsum(h, hpn.param[j]) 
				...
				for (MatrixEntry ee : list) {
					TMultiwayEntry e = (TMultiwayEntry)ee;
					t[e.t] += e.value * e.lexScore * h[e.h] * m[e.m] * dd[e.dd] * hc[e.hc] * mc[e.mc] * (e.label >= 0 ? l[e.label] : 1.0);
				}
				for (int j = 0; j < i; ++j) {
					double dot = lexSum[j]
							   * Utils.dotsum(h, hpn.param[j]) 
				...
				if (options.learnLabel) {
					...
					for (MatrixEntry ee : list) {
						TMultiwayEntry e = (TMultiwayEntry)ee;
						if (e.label >= 0) {
							l[e.label] += e.value * e.lexScore * h[e.h] * m[e.m] * hc[e.hc] * mc[e.mc] * dd[e.dd] * t[e.t];
						}
					}
					for (int j = 0; j < i; ++j) {
						double dot = lexSum[j]
				...
				for (MatrixEntry ee : list) {
					TMultiwayEntry e = (TMultiwayEntry)ee;
					hc[e.hc] += e.value * e.lexScore * m[e.m] * h[e.h] * mc[e.mc] * dd[e.dd] * (e.label >= 0 ? l[e.label] : 1.0) * t[e.t];
				}
				for (int j = 0; j < i; ++j) {
					double dot = lexSum[j] 
							   * Utils.dotsum(m, mpn.param[j]) 
				...
				for (MatrixEntry ee : list) {
					TMultiwayEntry e = (TMultiwayEntry)ee;
					mc[e.mc] += e.value * e.lexScore * h[e.h] * hc[e.hc] * m[e.m] * dd[e.dd] * (e.label >= 0 ? l[e.label] : 1.0) * t[e.t];
				}
				for (int j = 0; j < i; ++j) {
					double dot = lexSum[j]
							   * Utils.dotsum(m, mpn.param[j]) 
				...
				typoNorm = Math.sqrt(Utils.squaredSum(t));
				multiNorm = Math.sqrt(Utils.squaredSum(mc));
				if (lastTypoNorm != Double.POSITIVE_INFINITY && Math.abs(typoNorm-lastTypoNorm) < eps
					&& lastMultiNorm != Double.POSITIVE_INFINITY && Math.abs(multiNorm-lastMultiNorm) < eps) {
					break;
				}
				lastTypoNorm = typoNorm;
			...
			if (iter >= MAXITER) {
				System.out.printf("\tWARNING: Power method didn't converge." +
						"R=%d sigma=%f %f%n", i, typoNorm, multiNorm);
			}
			//if (Math.abs(svoNorm) <= eps || Math.abs(typoNorm) <= eps || Math.abs(multiNorm) <= eps) {
			//	System.out.printf("\tWARNING: Power method has nearly-zero sigma. R=%d%n",i);
			//}
			System.out.printf("  %.2f/%.2f", typoNorm, multiNorm);
		...
		if (options.updateMode == UpdateMode.MIRA) {
			for (int i = 0; i < maxRank; ++i) {
				...
				lpn.total[i] = lpn.param[i].clone();
				if (options.lexical) {
					hlpn.total[i] = hlpn.param[i].clone();
					mlpn.total[i] = mlpn.param[i].clone();
				}
			}
		}
	}
...
class TMultiwayEntry extends MatrixEntry
{
	public int hc, mc, t, hl, ml;
	public double lexScore;
	public double delexScore;
	public TMultiwayEntry(int _h, int _m, int _hc, int _mc, int _dd, int _label, int _t, int _hl, int _ml, double _value)
	{
		super(_h, _m, _dd, _label, _value);
		hc = _hc;
		mc = _mc;
