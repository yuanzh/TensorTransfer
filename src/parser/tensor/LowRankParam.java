...
public class LowRankParam {
	...
	public void putEntry(int h, int m, int hc, int mc, int dd, int label, int svo, int t, double v)
	{
		list.add(new HierarchicalEntry(h, m, hc, mc, dd, label, svo, t, v));
	}
			...
			for (iter = 0; iter < MAXITER; ++iter) {
				...
				for (MatrixEntry ee : list) {
					...
					if (e.t >= 0) {
						Utils.Assert(e.svo < 0);
						Utils.Assert(e.h < 0);
						t[e.t] += e.value * hc[e.hc] * mc[e.mc] * (e.label >= 0 ? l[e.label] : 1.0);
					}
				}
				for (int j = 0; j < i; ++j) {
					double dot = Utils.dotsum(hc, hcpn.param[j])
							   * Utils.dotsum(mc, mcpn.param[j])
							   * (lpn != null ? Utils.dotsum(l, lpn.param[j]) : 1.0);
				...
				if (options.learnLabel) {
					...
					for (MatrixEntry ee : list) {
						...
						if (e.svo >= 0) {
							...
							svo[e.svo] += e.value * hc[e.hc] * mc[e.mc];
						}
					}
					for (int j = 0; j < i; ++j) {
						double dot = Utils.dotsum(hc, hcpn.param[j])
					...
					for (MatrixEntry ee : list) {
						...
						if (e.label >= 0) {
							...
							if (e.h >= 0) {
								...
								l[e.label] += e.value * hc[e.hc] * mc[e.mc] * t[e.t];
							}
							else {
				...
				for (MatrixEntry ee : list) {
					...
					if (e.h >= 0) {
						...
						Utils.Assert(e.svo < 0);
						hc[e.hc] += e.value * mc[e.mc] * t[e.t] * (e.label >= 0 ? l[e.label] : 1.0);
					}
					else if (e.svo >= 0) {
						Utils.Assert(e.t < 0);
						Utils.Assert(e.label < 0);
						hc[e.hc] += e.value * mc[e.mc] * svo[e.svo];
					}
					else {
				...
				for (MatrixEntry ee : list) {
					...
					if (e.h >= 0) {
						...
						Utils.Assert(e.svo < 0);
						mc[e.mc] += e.value * hc[e.hc] * t[e.t] * (e.label >= 0 ? l[e.label] : 1.0);
					}
					else if (e.svo >= 0) {
						Utils.Assert(e.t < 0);
						Utils.Assert(e.label < 0);
						mc[e.mc] += e.value * hc[e.hc] * svo[e.svo];
					}
					else {
...
{
	public int hc, mc, svo, t;
	public HierarchicalEntry(int _h, int _m, int _hc, int _mc, int _dd, int _label, int _svo, int _t, double _value)
	{
		super(_h, _m, _dd, _label, _value);
		hc = _hc;
		mc = _mc;
